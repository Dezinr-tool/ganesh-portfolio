  ...
  return (
    <div>
      <canvas
        ...
        className={`touch-none border border-[var(--color-text)] bg-[var(--color-bg)] ${disabled ? "cursor-not-allowed opacity-60" : "cursor-crosshair"}`}
      />
      {showClearButton && !disabled ? (
        <button
          type="button"
          onClick={clear}
          className="mt-2 text-xs text-[var(--color-text)] hover:text-[var(--color-text)]"
        >
          Clear
        </button>
      ) : null}
