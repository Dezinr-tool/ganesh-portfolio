...
type AgreementRow = {
  ...
  deliverable_phases: DeliverablePhase[] | string;
...
function rowToAgreement(row: AgreementRow): Agreement {
  ...
  return {
    ...
    deliverablePhases: parseJson<DeliverablePhase>(row.deliverable_phases),
    totalTimeline: row.total_timeline ?? "",
...
export async function createAgreement(
  ...
  const { rows } = await sql<AgreementRow>`
    INSERT INTO agreements (
      ...
      ${JSON.stringify(input.deliverablePhases ?? [])},
      ${input.totalTimeline ?? ""},
